...
public sealed class ResolveTypesPass : AbstractPass
{
    ...
    public override void Visit(FunctionCallExpression e)
    {
        ...
        else if (e.Function is FunctionDeclaration userFunction)
        {
            ...
            {
                if (userFunction.Parameters[i].ResultType != e.Arguments[i].ResultType)
                {
                    throw new TypeMismatchException($"Function '{e.Name}' expects argument '{userFunction.Parameters[i].Name}' with type '{e.Arguments[i].ResultType}'");
                }
            }
        }
